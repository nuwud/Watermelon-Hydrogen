// Backup of simplified Carousel3DSubmenu created on 2025-09-07
// This variant was used temporarily to fix parse errors.
// Restored full-featured version from git HEAD afterwards.

// BEGIN SIMPLIFIED VERSION
// Carousel3DSubmenu: 3D submenu ring for Carousel3DPro
// Handles item layout, selection/highlight, scroll, and lifecycle.
...
// Minimal theme defaults for colors
const defaultTheme = {
  textColor: 0xffffff,
...
export class Carousel3DSubmenu extends THREE.Group {
  constructor(parentItem, items = [], config = {}) {
    super();
    console.warn('[🍉 Watermelon] Carousel3DSubmenu constructor', {
    ...
    // Load font (use cached when possible)
  ...
  loadFontSequentially(fontPaths, pathIndex) {
    ...
    this.fontLoader.load(
      currentPath,
      (font) => {
        this.font = font; cachedFont = font;
        this.createItems();
        this.addCloseButton();
        this.isInitialized = true;
        if (this.itemMeshes.length > 0) {
  ...
  createItems() {
    ...
    this.items.forEach((item, index) => {
      ...
      // Text
      const geometry = new TextGeometry(String(item), {
        font: this.font,
        size: 0.15,
        height: 0.02,
        curveSegments: 16,
  ...
  selectItem(index, animate = true) {
    ...
    if (animate) {
      gsap.to(this.itemGroup.rotation, {
        x: target, duration: 0.6, ease: 'power2.out', onComplete: () => { this.selectItemLock = false; }
      });
    } else {
  ...
  scrollSubmenu(delta) {
    if (this.selectItemLock || this.isAnimating) return;
    const angleStep = (2 * Math.PI) / this.itemMeshes.length;
    this.targetRotation += delta > 0 ? -angleStep : angleStep;
    this.isAnimating = true;
    gsap.to(this.itemGroup.rotation, {
      x: this.targetRotation, duration: 0.3, ease: 'power3.out', onComplete: () => {
        this.isAnimating = false; this.updateFrontItemHighlight();
      }
    });
  }

  updateFrontItemHighlight() {
    let frontIndex = 0; let minDistance = Infinity; const tmp = new THREE.Vector3();
    this.itemMeshes.forEach((item, index) => {
      item.getWorldPosition(tmp); const distance = Math.abs(tmp.z - this.watermillRadius);
  ...
  show() {
    // Animate in items and button; return a promise to integrate with manager
    return new Promise((resolve) => {
      // Scale items up from 0
      this.itemMeshes.forEach((container, i) => {
        const mesh = container.userData.mesh; if (!mesh) return;
        mesh.scale.set(0, 0, 0);
        gsap.to(mesh.scale, { x: mesh.userData.originalScale.x, y: mesh.userData.originalScale.y, z: mesh.userData.originalScale.z, duration: 0.25, delay: i * 0.03, ease: 'back.out' });
      });
      if (this.closeButton) {
        this.closeButton.scale.set(0.5, 0.5, 0.5);
        gsap.to(this.closeButton.scale, { x: 1, y: 1, z: 1, duration: 0.25, delay: 0.15, ease: 'back.out' });
      }
      // Ensure first item highlighted and wheel aligned
  ...
  hide() {
    return new Promise((resolve) => {
      this.itemMeshes.forEach((container, i) => {
        const mesh = container.userData.mesh; if (!mesh) return;
        gsap.to(mesh.scale, { x: 0, y: 0, z: 0, duration: 0.2, delay: i * 0.02, ease: 'back.in' });
      });
      if (this.closeButton) {
        gsap.to(this.closeButton.scale, { x: 0, y: 0, z: 0, duration: 0.2, ease: 'back.in' });
      }
  ...
  update() {
    // Follow parent position/orientation
    try {
      if (this.parentItem && this.parentItem.parent) {
        ...
        const parentRot = this.parentItem.rotation?.y; const grandParentRot = this.parentItem.parent.rotation?.y;
        if (!isNaN(parentRot)) this.rotation.y = isNaN(grandParentRot) ? parentRot : grandParentRot + parentRot;
      }

      // Counter-rotate containers to face camera-ish and keep on ring
      ...
      // Drift toward target rotation when not tweening
      if (!this.isAnimating && !gsap.isTweening(this.itemGroup.rotation)) {
        const twoPi = Math.PI * 2; const norm = (a) => ((a % twoPi) + twoPi) % twoPi;
        const current = norm(this.itemGroup.rotation.x); const target = norm(this.targetRotation);
        const diff = target - current; const threshold = 0.005;
        if (Math.abs(diff) > threshold) {
          this.itemGroup.rotation.x += diff * this.rotationSpeed;
        }
      }

      // Close button face camera if available
      if (this.closeButton && this.camera) {
        const cameraPosWorld = new THREE.Vector3().copy(this.camera.position);
      ...
      console.error('[Carousel3DSubmenu] update error', err);
    }
  }
  ...
  dispose() {
    try {
      ...
      this.itemMeshes.forEach((container) => {
        const mesh = container.userData?.mesh; const hit = container.userData?.hitArea;
        if (mesh) { mesh.geometry?.dispose(); mesh.material?.dispose(); }
        if (hit) { hit.geometry?.dispose(); hit.material?.dispose(); }
      ...
      if (this.closeButton) {
        ...
        this.closeButton.userData?.xLines?.forEach((line) => { line.geometry?.dispose(); line.material?.dispose(); });
      ...
      console.error('[Carousel3DSubmenu] dispose error', e);
    }
  }
}
