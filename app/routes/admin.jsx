    ...
    <div style={styles.container}>
      ...
      <div style={styles.main}>
        ...
        <nav style={styles.sidebar}>
          {['scene', 'camera', 'carousel', 'submenu', 'mobile', 'scatter', 'hud'].map(tab => (
            <button
              key={tab}
        ...
        <div style={styles.configPanel}>
          ...
          {activeTab === 'hud' && (
            <ConfigSection title="Camera HUD">
              <ToggleInput
                label="Enable HUD"
                value={config.hud.enabled}
                onChange={v => updateConfig('hud', 'enabled', v)}
              />
              <SliderInput
                label="HUD Radius"
                value={config.hud.radius}
                min={1.5}
                max={5}
                step={0.25}
                onChange={v => updateConfig('hud', 'radius', v)}
              />
              <SliderInput
                label="Opacity"
                value={config.hud.opacity}
                min={0.5}
                max={1}
                step={0.05}
                onChange={v => updateConfig('hud', 'opacity', v)}
              />
              <div style={{marginBottom: '15px'}}>
                <span style={{display: 'block', marginBottom: '5px', fontSize: '12px', opacity: 0.8}}>Cart Position</span>
                <select
                  value={config.hud.cartPosition}
                  onChange={e => updateConfig('hud', 'cartPosition', e.target.value)}
                  style={{width: '100%', padding: '8px', background: '#1a1a2e', color: 'white', border: '1px solid #333', borderRadius: '4px'}}
                >
                  <option value="TOP_RIGHT">Top Right</option>
                  <option value="TOP_LEFT">Top Left</option>
                  <option value="BOTTOM_RIGHT">Bottom Right</option>
                  <option value="BOTTOM_LEFT">Bottom Left</option>
                  <option value="CART">Cart (default)</option>
                </select>
              </div>
              <button
                onClick={() => {
                  if (window.__wmCartHUDIcon) {
                    window.__wmCartHUDIcon.animateItemAdd();
                  }
                }}
