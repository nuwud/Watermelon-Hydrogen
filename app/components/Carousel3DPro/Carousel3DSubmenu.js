...
export class Carousel3DSubmenu extends THREE.Group {
  ...
  createItems() { // Check if font is loaded
    ...
    const angleStep = (2 * Math.PI) / this.items.length; // Calculate angle step between items
    ...
    this.items.forEach((item, index) => { // Loop through each item in the submenu
      ...
      const angle = angleStep * index; // Calculate angle for this item
      //container.position.x = this.watermillRadius * Math.cos(angle); // Position based on angle
      container.position.y = this.watermillRadius * Math.sin(angle); // Position based on angle 
      container.position.z = this.watermillRadius * Math.cos(angle); // Position based on angle
