...
export class Carousel3DSubmenu extends THREE.Group { // Class definition for Carousel3DSubmenu
  ...
  constructor(parentItem, items = [], config = {}) { // Call parent constructor
    ...
    this.mainCarouselHomeAngle = typeof config.mainCarouselHomeAngle === 'number'
      ...
      : 0;    this.rotationAngle = 0; // Current rotation angle of the submenu
    ...
    this.lastParentRotation = 0; // Initialize to track parent rotation    // Initialize selection guard for robust state management
    // Use provided guard from config, or create a new one
    this.guard = config.guard || new SelectionGuard();
    this.guard.enableDebug(); // Enable debug mode for better diagnostics
    
    // Legacy lock flags - these will be replaced by guard system
    ...
    if (cachedFont) { // Check if font is already cached
      ...
      console.warn('[Carousel3DSubmenu] Items created from cached font:', this.itemMeshes.length);      // Position wheel so first item is at front
      if (this.itemMeshes.length > 0) { // Check if there are items created
        ...
        this.targetRotation = this.itemGroup.rotation.x; // Set target rotation to match        // Ensure the first item is highlighted
        this.currentIndex = 0;
    ...
    this.fixedElements = new THREE.Group(); // Create a separate group for fixed elements    this.add(this.fixedElements); //  Add to the main group
 ...
 */
  ...
  /**
   * Gets a color from a modern, accessible palette based on the provided index.
   * Enhanced with better contrast and visual appeal.
   *
   * @param {number} index - The index to determine the color.
   * @returns {number} The hexadecimal color code corresponding to the index.
   */  getIconColor(index) {
    // IMPROVEMENT: Modern, accessible color palette with enhanced contrast and visual appeal
    const colors = [
      0x3B82F6, // Bright blue - more vibrant than before
  ...
  /**
   * Gets a comprehensive modern color palette including text, emissive, and accent colors.
   * Enhanced with better contrast ratios and accessibility considerations.
   *
   * @param {number} index - The index to determine the color scheme.   * @returns {Object} Object containing text, emissive, and accent colors.
   */
  getModernColorPalette(index) {
    // Cache the palettes to avoid recreating the array on every call (performance optimization)
    if (!this.cachedPalettes) {
      this.cachedPalettes = [
        ...
        { // Forest theme - natural and calming
          text: 0xF0FDF4,     // Very light green for text
          emissive: 0x166534, // Dark forest green for glow
          accent: 0x22C55E    // Bright green for icons
        },
        { // Royal Purple theme - elegant and modern
          text: 0xFAF5FF,     // Very light purple for text
          emissive: 0x7C2D12, // Dark purple-brown for glow
          accent: 0xA855F7    // Bright purple for icons
        },
        { // Cyber theme - futuristic feel
          text: 0xECFEFF,     // Light cyan for text
          emissive: 0x164E63, // Dark teal for glow
          accent: 0x06B6D4    // Bright cyan for icons
        },
        { // Golden theme - luxury feel
          text: 0xFFFBEB,     // Cream for text
          emissive: 0x92400E, // Dark amber for glow
          accent: 0xF59E0B    // Gold for icons
        },
        { // Rose theme - elegant and soft
          text: 0xFFF1F2,     // Very light pink for text
          emissive: 0xBE123C, // Deep rose for glow
          accent: 0xF43F5E    // Bright rose for icons
        },
        { // Electric theme - high energy
          text: 0xF7FEE7,     // Light yellow-green for text
          emissive: 0x365314, // Dark lime for glow
          accent: 0x84CC16    // Electric lime for icons
        }
      ];
    }
    
    return this.cachedPalettes[index % this.cachedPalettes.length];
  }
  ...
  createItems() { // Ensure font is loaded before creating items
    ...
    const galleryShapes = { // Gallery-specific shapes
    ...
    };    this.items.forEach((item, index) => { // Iterate through each item in the submenu      // IMPROVEMENT: Enhanced dynamic text sizing with better scaling curve
      ...
      // Create text geometry with dynamic sizing
      const geometry = new TextGeometry(itemText, { // Create text geometry using the loaded font
        font: this.font, // Use the loaded font
        size: fontSize, // Dynamic font size based on content length
      ...
      // IMPROVEMENT: Enhanced color scheme with better contrast and modern palette
      const modernColors = this.getModernColorPalette(index);
      
      // Create material for text with improved colors
      const material = new THREE.MeshStandardMaterial({ // Create a standard material for the text
        color: modernColors.text, // Modern text color
        transparent: true, // Enable transparency
        opacity: 0.95, // Slightly higher opacity for better readability
        emissive: modernColors.emissive, // Modern emissive color
        emissiveIntensity: 0.3, // Slightly higher emissive intensity
        metalness: 0.1, // Add subtle metallic look
        roughness: 0.3 // Smooth but not too shiny
      });
      ...
      hitArea.position.z = 0; // Center the hit area around the text
      hitArea.userData.isHitArea = true; // Mark as hit area for debugging
      hitArea.userData.isSubmenuItem = true; // Ensure this gets picked up by click handlers
      ...
      if (item === 'Cart') {
        ...
        iconMesh.scale.set(0, 0, 0); // Start invisible for animation      } else {
        // Use regular shape (synchronous) with modern color palette
        ...
        // IMPROVEMENT: Use modern color palette for icons
        const modernColors = this.getModernColorPalette(index);
        
        const shapeMaterial = new THREE.MeshStandardMaterial({ // Create a standard material for the icon shape
          color: modernColors.accent, // Use modern accent color
          metalness: 0.4, // Slightly more metallic for modern look
          roughness: 0.2, // Smoother for more polished appearance
          emissive: modernColors.emissive, // Use modern emissive color for glow effect
          emissiveIntensity: 0.25 // Slightly higher emissive intensity
  ...
  highlightItemAtIndex(index) {
    ...
    // Apply highlight to the target item using modern color palette
    const container = this.itemMeshes[index];
    const mesh = container.userData.mesh;
    const iconMesh = container.userData.iconMesh;
    
    // Get modern colors for this item
    const modernColors = this.getModernColorPalette(index);

    if (mesh) {
      // Use modern highlight colors for better visual appeal
   ...
   */  selectItem(index, animate = true, createPreview = false) {
    ...
    // Use the guard system for all locking - this replaces the manual locks
    return withSelectionLock(this.guard, index, () => {
      ...
      // Choose the shorter path - this ensures perfect butter-smooth behavior
      ...
      // Unhighlight previous item if different
      ...
      // Handle preview creation if requested
      if (createPreview) {
   ...
   */  scrollSubmenu(delta) { // Scroll the submenu by a specified delta value
    // Use guard system to check if scrolling is allowed
    if (!this.guard.canScroll()) {
      console.warn("🚫 scrollSubmenu blocked by guard");
      return;
    }
    ...
    gsap.to(this.itemGroup.rotation, {
      ...
      onUpdate: () => {
        // Update the highlight during the animation if guard allows
        if (this.guard.canUpdateHighlight(false)) {
          this.updateFrontItemHighlight();
        }
      },
      onComplete: () => { // Animation complete callback
        this.isAnimating = false; // <<< CLEAR ANIMATION FLAG
        // Force highlight update on completion
...
update() {
  try {
    ...
    if (Math.abs(diff) > threshold) {
      ...
      // 🛡️ Final override protection using guard system
      const recentlySelected = (Date.now() - (this.lastSelectTimestamp || 0)) < 500;

      if (this.guard.canUpdateHighlight(false) && !recentlySelected && !gsap.isTweening(this.itemGroup.rotation)) {
        const frontIndex = this.getFrontIndex();
        if (frontIndex !== -1 && frontIndex !== this.currentIndex) {
  ...
  updateFrontItemHighlight(force = false) { // Update the highlight of the item currently at the front (3 o'clock position)
    // Use guard system to check if highlighting is allowed
    if (!this.guard.canUpdateHighlight(force)) {
      return;
    }
   ...
   * Handles click events on submenu items - simplified version
   */
